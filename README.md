...
### To fix the cors error we  can add ngnix.conf file were the server is located 

we set up a proxy server and allow all suitable  headers to remove cors err.
...
server {
    ...
    location / {
        proxy_pass http://<external_url>:5000;  # Replace with the internal URL of your API
...
and forward the port for image gen model to port 8080 and run the docker container
